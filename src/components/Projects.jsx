...
const Card = ({project}) => {
    return (
        <div className="lg:flex gap-4 rounded-lg bg-slate-800/50 p-8">
            <img src={project.imgSrc} alt={project.title} className="w-32 h-18 rounded-md border-slate-700 border-solid border-2 flex-shrink-0 mb-4"/>
            <div>
                <div className="flex justify-between">
                <h3 className="text-lg text-slate-200 leading-tight font-medium ">{project.title}</h3>
                <div className="flex gap-4">
                    <a href={project.sourceCode} target='_blank' rel='noreferrer noopener' title={project.title}
                    className="group">
                ...
                <ul className="flex gap-2 mt-4">
                    {project.tags.map((tag, index) => (
                        <li key={index+1}
                        className="text-cyan-500 bg-cyan-400/20 rounded-full px-2 py-1 text-xs font-bold">{tag}</li>
                    ))}
                </ul>
            </div>
        </div>
    ...
    const [projects, setProjects] = useState([
        {
            id: 1,
            title: "CV App",
            description: "Create a basic resume with personal information, education background, work experience, and skills.",
        ...
        },
        {
            ...
            description: "Write a message then post it. Also, view messages posted by other users.",
        ...
        },
        {
            id: 3,
            title: "Weather App",
            description: "Search for a city to view its current weather. This weather app uses the Visual Crossing free weather API to fetch data.",
        ...
        },
        {
            ...
            description: "A reading assistive device using Raspberry Pi and with Magnificaiton, Text-to-Speech, and Voice Command features.",
